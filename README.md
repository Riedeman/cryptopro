...
### Install Dependencies: Node and MySQL (example uses a clean debian 9 vm)
...
database: MySQL connection settings (same as create-database.sql script)
...
### Level 4 - OCD: Obsessive Cryptocurrency Disorder (Expert level only) (Also, not free: $19.99) (coming soon)
...
Cryptopro uses asynchronous processes and stored procedures together to react as quickly as possible to any arbitrage opportunities it discovers. Despite that, sometimes markets move quickly away from the last orderbook buy/sell price causing an order to stay open until the market returns to that price point. In that case, one side of the trade may get filled, but the other will be unresolved.  If you chose to close the order on the exchange instead of leaving it open in the hopes the market will return to that price point, one of the "legs" of the trade will be unresolved.
